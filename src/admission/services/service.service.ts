...
export class ServiceService {
  ...
  async create(body) {
    delete body._id;
    const newEntity = new this.serviceModel(body);
  ...
  async read(key, val) {
    try {
      return await this.serviceModel.findOne({ [key]: val });
    } catch (err) {
      return null;
    }
  }
  ...
  async update(id: ObjectId, set): Promise<Service> {
  ...
  async deleteById(id: string) {
    const deleted = await this.serviceModel.findByIdAndRemove(id);
    return deleted;
  }

  async deleteEntities(listOfIds: string[]) {
    return this.serviceModel.deleteMany({ _id: { $in: listOfIds } });
  }
  ...
  async godSearch(limit: number, page: number, filters?: any, sortBy?: object) {
    ...
    const aggregationResults = await this.serviceModel
      .aggregate()
