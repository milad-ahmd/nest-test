...
export class MethodService {
  ...
  async create(body) {
    delete body._id;
    const newEntity = new this.methodModel(body);
    await newEntity.save();
    return newEntity;
  }

  async readAll(): Promise<Method[]> {
    return await this.methodModel.find().exec();
  }

  async read(key, val) {
    try {
      return await this.methodModel.findOne({ [key]: val });
    } catch (err) {
      return null;
    }
  }

  async update(id: ObjectId, set): Promise<Method> {
    const updated = await this.methodModel.findOneAndUpdate({ _id: id }, set, {
      new: true,
    });
    return updated;
  }
  ...
  async deleteById(id: string) {
    const deleted = await this.methodModel.findByIdAndRemove(id);
    return deleted;
  }

  async deleteEntities(listOfIds: string[]) {
    return this.methodModel.deleteMany({ _id: { $in: listOfIds } });
  }
  ...
  async godSearch(limit: number, page: number, filters?: any, sortBy?: object) {
    ...
    const aggregationResults = await this.methodModel
      .aggregate()
